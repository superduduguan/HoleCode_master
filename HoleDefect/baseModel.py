...
class BaseModel(object):
    ...
    def __build_train_op(self):
        ...
        with tf.name_scope('loss'):
            self.mygt = tf.cast(tf.one_hot(tf.cast(self.class_gt, dtype=tf.uint8), 10), dtype=tf.float32)   #classnum
            # print(self.mygt)
            # print(self.pred)
            self.class_loss = tf.reduce_mean(
                tf.nn.sigmoid_cross_entropy_with_logits(labels=self.mygt, logits=self.pred))  
                    ...
                    self.writer.add_summary(
                        self.sess.run(self.summ_scalar,
                                      feed_dict={self.img: _test_batch[0],
                                                 ...
                                                 self.class_gt: _test_batch[1]}), _iter_count)
